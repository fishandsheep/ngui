...
description: 面向 Nginx 路由分析与配置检查的紧凑型本地工作台
colors:
  graphite-bg: "#0b1020"
...
界面应像一台专门用于追踪请求路径和审查配置质量的网络控制台：信息密集但不拥挤，技术感来自真实数据、结构和状态，而不是装饰。三栏工作区把配置输入、拓扑画布和对象详情放在稳定位置，用户可以持续编辑、定位、验证和导出路由关系。

系统以深色模式为主要工作环境，同时提供语义一致的浅色模式。视觉层级主要依靠背景色阶、细边框、字号和留白建立。避免营销页式表达、霓虹渐变、装饰性玻璃效果、过度卡片化以及与状态无关的动画。

当前产品除拓扑外，还承担解析错误、静态配置建议和双语文案切换，因此设计系统必须把“状态语义一致性”与“文本可读性”视为第一优先级，而不是次要补充。
...
石墨深蓝构成安静的工作环境，路由绿用于主要交互和选择，拓扑蓝用于常规连接关系，琥珀与红色保留给警告和错误，紫色和橙色仅用于特定拓扑语义。

### Primary

- **路由绿** (`#49d6a6`，浅色 `#138a68`)：品牌标记、交互强调、搜索命中和选中状态。单个视图中应保持克制，不作为大面积装饰背景。

### Secondary

- **拓扑蓝** (`#5ea2ff`，浅色 `#2368d4`)：常规请求流、结构层级和中性技术强调。

### Tertiary

- **警告琥珀** (`#f7bd5c`，浅色 `#a66104`)：配置警告、动态目标和低置信度信息。
- **危险红** (`#ff7b7b`，浅色 `#b82e39`)：解析错误和破坏性反馈，不用于普通强调。
- **信息紫** (`#e066ff`)：重写链路和次级技术标记，仅用于有明确类型区分的元素。
- **变量橙** (`#ff8f70`)：变量节点和目标解析相关语义，不可扩散为通用强调色。
...
**状态颜色规则。** 颜色必须与文字、图标、形状或线型共同表达状态，不允许只靠颜色区分节点类型、错误、警告、提示或选中状态。Issue 列表中的 `error / warning / info` 必须先由文案表达，再由颜色辅助。
...
- **Body**（400，13px，1.55）：说明、详情内容、问题建议和列表。
- **Label**（400，11–12px，1.35）：控件标签、节点类型、状态和元数据；大写仅用于短类型标记。
- **Code**（400，12px，1.55）：配置编辑器、原始指令和代码片段。

**工具字体规则。** 控件、标签和数据使用固定 rem/px 层级，不使用流式 `clamp()` 大标题；长说明控制在 65–75ch 内；双语切换后按钮和标签长度变化不得破坏布局。
...
- **Role:** 左侧问题区是“解析错误 + 配置建议”的统一入口，不是装饰性提示。
- **Density:** 最多优先显示 5 条高价值问题；排序固定为 `error > warning > info`。
- **Formatting:** 每条使用 `"[Severity] L{line}: message"` 的稳定结构，建议文本单独换行，不嵌套复杂视觉层次。
- **Semantics:** `error` 代表解析或结构级阻断，`warning` 代表高价值风险，`info` 代表保守建议。颜色只辅助，不替代文案。
...
编辑层与高亮层重叠，使用 12px 等宽字体和 1.55 行高。块、指令、变量和目标使用既定语义颜色；光标、选区和滚动位置必须始终清晰可见。上传文件、粘贴文本和手工编辑都应获得一致反馈。

## 6. Do's and Don'ts

### Do:

- **Do** 保持画布为主要工作区，配置、检查结果和详情面板服务于路由理解。
- **Do** 在深浅主题和中英文界面中保持相同的状态语义和信息层级。
...
- **Do** 把乱码、翻译不一致和状态文案缺失视为设计系统问题，而不是单纯实现细节。
...
- **Don't** 发明偏离开发者工具习惯的交互方式，也不要隐藏导入、搜索、布局、检查和导出等关键操作。
- **Don't** 牺牲信息密度、文本可读性、双语稳定性或拓扑辨识度来追求视觉效果。
